namespace Conquest.scripts
{
    ...
    public static class ConnectionHelper
    {
        ...
        public static void SendMessageToServer(MessageBase message)
        {
            ...
            try
            {
                ...
                if (byteData.Length <= MAX_MESSAGE_SIZE)
                    MyAPIGateway.Multiplayer.SendMessageToServer(ConquestConsts.ConnectionId, byteData);
                else
        ...
        {
            ConquestScript.Instance.ServerLogger.WriteVerbose("SendMessageToPlayer {0} {1} {2}.", steamId, message.Side, message.GetType().Name);
            ...
            try
            {
                ...
                byte[] byteData = System.Text.Encoding.Unicode.GetBytes(xml);
                if (byteData.Length <= MAX_MESSAGE_SIZE)
                    MyAPIGateway.Multiplayer.SendMessageTo(ConquestConsts.ConnectionId, byteData, steamId);
                else
        ...
        {
            ConquestScript.Instance.ServerLogger.WriteStart("Start Message Serialization");
            ...
            {
                ConquestScript.Instance.ServerLogger.WriteError("Message cannot Deserialize");
                return;
            }

            ConquestScript.Instance.ServerLogger.WriteStop("End Message Serialization");
            ...
            if (message != null && message.Content != null)
            {
                ...
                {
                    ConquestScript.Instance.ServerLogger.WriteError("Processing message exception. Side: {0}, Exception: {1}", message.Content.Side, e.ToString());
                }
                return;
            }
        }
        ...
        public static int GetFreeByteElementCount(MessageIncomingMessageParts message)
        {
            ...
            ConquestScript.Instance.ServerLogger.WriteVerbose(string.Format("FinalLength: {0}", finalLength));
        ...
        {
            ConquestScript.Instance.ServerLogger.WriteVerbose("SendMessageParts {0} {1} {2}.", byteData.Length, side, receiver);
                ...
                try
                {
                    ...
                    switch (side)
                    {
                        case MessageSide.ClientSide:
                            if (MyAPIGateway.Multiplayer.SendMessageTo(ConquestConsts.ConnectionId, bytes, receiver))
                                byteList.RemoveRange(0, count);
                            else
                                throw new Exception("Failed to send message parts to client.");
                            break;
                        case MessageSide.ServerSide:
                            if (MyAPIGateway.Multiplayer.SendMessageToServer(ConquestConsts.ConnectionId, bytes))
                                byteList.RemoveRange(0, count);
                            else
